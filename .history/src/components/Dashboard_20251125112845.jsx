...
export default function Dashboard() {
  ...
  const deleteTask = id => {
    if(window.confirm("Are you sure to delete?")) setTasks(prev => prev.filter(t => t.id !== id));
  };
  ...
  // Mock Email Automation every 20 min
  useEffect(() => {
    const interval = setInterval(() => {
      const pending = tasks.filter(t => t.status === "Pending");
      if(pending.length) console.log("Mock Email Notification:", pending);
    }, 1200000);
